...
## PR 5: ElevenLabs sound (2026-09-14)

- `POST /v1/speech` (JSON: `text`, `voice_id` required; `model` one of `eleven_multilingual_v2` (default), `eleven_v3`, `eleven_flash_v2_5`; `language_code`; `seed`), `POST /v1/music` (JSON: `prompt`, `length_ms` 3,000 to 600,000 required; `instrumental`; `seed`), and free `GET /v1/speech/voices`. Everything comes back as MP3 at 44.1 kHz, 128 kbit/s; anything that is not MP3 counts as a failure and is not settled.
- **No sound effects**, as step 0 found: ElevenLabs' Prohibited Use Policy 9(c) bans reselling them on their own.
- **Price = what ElevenLabs will charge us + 25%, floor $0.005.** Speech is $0.10 per 1,000 characters for multilingual v2 and v3 and $0.05 for flash v2.5; music is $0.15 per minute (API pricing page, 2026-09-14). A 10-second voice-over of about 150 characters costs the floor; 10 seconds of music costs us $0.025 and sells for $0.03125. The cost counts against the daily cap.
- Text limits per request follow ElevenLabs' model limits: 10,000 characters for multilingual v2, 5,000 for v3, 40,000 for flash v2.5. The 2,000-character music prompt cap is ours.
